...
export function ProductCard({ product, index, officialDollarRate }: { product: Product; index: number; officialDollarRate?: OfficialDollarRate }) {
  ...
  return (
    <article className={`product-card group ${prominent ? "product-card--prominent" : ""}`}>
      <div className="flex flex-col items-center justify-center gap-3 text-center sm:flex-row sm:items-start sm:justify-between sm:gap-5 sm:text-left">
      ...
      <div className="mt-12 flex flex-col items-center justify-center gap-6 text-center sm:flex-row sm:items-end sm:justify-between sm:text-left md:mt-16">
        <div className="w-full">
          <div className="mx-auto mb-6 grid size-12 place-items-center rounded-xl border border-current/15 bg-current/[0.035] transition-transform duration-300 group-hover:-rotate-3 group-hover:scale-105 sm:mx-0">
          ...
          <p className="mx-auto mt-4 max-w-[29rem] text-[15px] leading-6 opacity-65 sm:mx-0">{product.shortDescription}</p>
        </div>
      ...
      <div className="mt-10 grid grid-cols-2 gap-3 border-t border-current/12 pt-5">
        <div className={`min-h-[92px] rounded-lg border px-3 py-2 text-center sm:text-left ${referencePriceStyle}`}>
        ...
        <div className={`min-h-[92px] rounded-lg border px-3 py-2 text-center sm:text-right ${kuentraPriceStyle}`}>
